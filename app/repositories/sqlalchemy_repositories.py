...
from ..utils.exceptions import RepositoryError, NotFoundError
...
class SQLAlchemyProfileRepository(ProfileRepository):
    ...
    def save(self, profile: Profile) -> None:
        try:
            profile_model = self.session.query(ProfileModel).filter(ProfileModel.user_id == profile.user_id).first()
            if profile_model:
                # update
    ...
    def get_by_user_id(self, user_id: UUID) -> Optional[Profile]:
        try:
            profile_model = self.session.query(ProfileModel).filter(ProfileModel.user_id == user_id).first()
